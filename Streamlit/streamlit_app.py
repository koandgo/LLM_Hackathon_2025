...
def rewrite_asr_cells_for_streamlit(nb, audio_var_name="AUDIO_FILE", require_openai=True):
    ...
    for i, cell in enumerate(list(nb.cells)):
        ...
        if ("import nemo.collections.asr" in src) or ("ASRModel.from_pretrained" in src) or ("from google.colab import files" in src) or ("files.upload(" in src):
            asr_hit = True
            nb.cells[i]["source"] = f"""
...
def execute_notebook(repo_dir: Path, notebook_rel_path: str, params: dict, timeout_s: int = 1800):
    ...
    try:
        hit = rewrite_asr_cells_for_streamlit(nb, audio_var_name='AUDIO_FILE')
