# streamlit_app.py
# Streamlit wrapper to (1) transcribe audio with OpenAI and (2) optionally run the repo notebook.
# - Transcribes uploaded audio via OpenAI (gpt-4o-mini-transcribe / gpt-4o-transcribe / whisper-1)
...
st.title("🧪 Speech → ELN")
st.caption("Transcribe audio via OpenAI, then (optionally) run the repo notebook to build ELN artifacts.")
...
with st.sidebar:
    ...
    model = st.selectbox("Model", ["gpt-4o-mini-transcribe", "gpt-4o-transcribe", "whisper-1"], index=0,
                         help="OpenAI audio transcription models")
    language = st.text_input("Language hint (optional, e.g., 'en')", value="")
    prompt = st.text_area("Custom vocabulary / prompt (optional)", help="Helps with jargon, names, or style.")
...
def transcribe_with_openai(audio_path: Path, api_key: str, model: str, language: str = "", prompt: str = "") -> str:
    """Transcribe audio file with OpenAI Audio API and return the text."""
    # Choose client
    ...
    # Whisper/gpt-4o-* transcribe responses expose .text
...
if run_btn:
    ...
    status.info("Transcribing with OpenAI…")
    try:
        transcript_text = transcribe_with_openai(tmp_audio, api_key=os.getenv("OPENAI_API_KEY"), model=model, language=language, prompt=prompt)
    ...
    params = {
        ...
        "TRANSCRIPT_FILE": transcript_path,      # new: path to transcript.txt
        "TRANSCRIPT_TEXT": transcript_text,      # new: raw transcript text
